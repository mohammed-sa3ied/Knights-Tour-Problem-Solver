"""
Genetic Algorithm (Classic) for Knight's Tour
...
class IndividualClassic:
    """Individual with random initialization."""
    ...
    def _calculate_fitness(self):
        """Calculate fitness as number of valid moves."""
        valid_count = 0
...
class PopulationClassic:
    """Population for GA Classic."""
    
    def __init__(self, board, size=100):
        self.board = board
        self.size = size
        self.individuals = [IndividualClassic(self.board) for _ in range(self.size)]
    ...
    def evolve(self, cx_prob=0.7, mut_prob=0.2, elite_ratio=0.1):
        ...
        while len(new_pop) < self.size:
            ...
            if random.random() < cx_prob:
                ...
                child1 = IndividualClassic(self.board, child1_chromo)
                child2 = IndividualClassic(self.board, child2_chromo)
            else:
...
class GeneticAlgorithmClassic:
    ...
    def __init__(self, board_size=8, pop_size=100, generations=500, 
        ...
        self.population = PopulationClassic(self.board, pop_size)
